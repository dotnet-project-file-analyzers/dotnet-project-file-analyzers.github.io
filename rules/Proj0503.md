...
If a third-party package license comes with a custom license (other then one
specified by the [SPDX](https://spdx.dev)), this rule ensures that the license
must have been aproved.

To aprove a license, a `<<ThirdPartyLicense>` node has to be added with a
